                            ...
                            <p><strong>Attachment:</strong> <a href={`http://localhost:2005${item.filePath}`} download target="_blank" rel="noopener noreferrer">View Attachment</a></p>
                                    ...
                                    {popUp === item._id && ( 
                                        <div className="solution">
                                            ...
                                            <textarea id="txt" className="inputs em-pas sol" onChange={e => handleTextArea(e)} value={textArea.solution} ></textarea>
                                            <button className="chat-btn sol" onClick={() => handlePost(item._id)} >Post</button>
                                        </div>
                                    )}
                             </>
                           )}
                           <img src={clickedId === item._id ? arrowUpDark : arrowDark} onClick={() => {
                                handleClick(item._id)
                                {clickedId === item._id && handleSolved(item._id)}
                                }} className="arrow" alt="down arrow" />
                        </div>
                        </div>
                    )
                } else {
